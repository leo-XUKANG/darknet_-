 ...
 *nets  : the network wich you will train
 n :  the number of gpu which you have used
 data d : the input data
 interval : which is an int number, when the ngpus >1 , we will sync the net in each 'interval'  batch
 */ 
    int i;
