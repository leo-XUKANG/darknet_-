...
void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear, int dont_show)
{
    ...
    args.c = net.c;
    args.paths = paths;
    args.n = imgs;//the image number of loads at a time
    args.m = plist->size;//the number of the all train image data
    ...
    args.threads = 16;    // 64 
//the number of threads ,you can set as 64 or set as 16,the bigger number will accelerate the training 
    ...
    //this will draw the train chart
#endif    //OPENCV
    ...
    while(get_current_batch(net) < net.max_batches){
        ...
        if(l.random && count++%10 == 0){
            ...
            //this is different from the original code,the original code is an random number form {320,352,...,608}
            //but this way will range(32,576) when the input is 416
            ...
            for(i = 0; i < ngpus; ++i){
                resize_network(nets + i, dim_w, dim_h);//要调整网络
            }
            net = nets[0];
        }
